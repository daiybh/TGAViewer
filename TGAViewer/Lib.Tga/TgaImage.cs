...
namespace Lib.Tga
{
    ...
    public sealed class TgaImage
    {
        ...
        /// </summary>
        private readonly bool useAlphaChannelForcefully_;
            ...
            switch (Header.ImageType)
            {
                ...
                case ImageTypes.CompressedTrueColor:
                    {
                        switch (Header.PixelDepth)
                        {
                            ...
                            default:
                                throw new NotSupportedException($"Color depth isn't supported({Header.PixelDepth}bpp).");
                        }
                    }
                ...
                default:
                    throw new NotSupportedException(
                        $"Image type \"{Header.ImageType}({ImageTypes.ToFormattedText(Header.ImageType)})\" isn't supported.");
            }
        }
        ...
        private void ReadUncompressedData(BinaryReader reader)
        {
            ...
            ImagePtr = ImageBytes.BytesToIntptr();
            if (Header.ImageDescriptor == 0x08)
            {
                int lineSize = Header.Width * GetBytesPerPixel();
                byte[] tempBytes = new byte[ImageBytes.Length];
                IntPtr tempIntPtr = tempBytes.BytesToIntptr();

                for (int i = 0; i < Header.Height / 2; i++)
                {
                    unsafe
                    {
                        Buffer.MemoryCopy((void*)(ImagePtr + i * lineSize), (void*)tempIntPtr, lineSize, lineSize);
                        Buffer.MemoryCopy((void*)(ImagePtr + (Header.Height - 1 - i) * lineSize), (void*)(ImagePtr + i * lineSize), lineSize, lineSize);
                        Buffer.MemoryCopy((void*)(tempIntPtr), (void*)(ImagePtr + (Header.Height - 1 - i) * lineSize), lineSize, lineSize);
                    }
                }
            }
        }
